...
import { fetchProductBySlug, WHATSAPP } from '@/lib/products'
import { SizeSelector } from './SizeSelector'

// Always render server-side so product data is always fresh from the DB
export const dynamic = 'force-dynamic'

/** Placeholder gallery slot */
...
export default async function ProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const accent = product.category_accent ?? '#6b7280'
  const displayName = product.variant ? `${product.name} — ${product.variant}` : product.name
  const images = product.images ?? []

  // Gallery: up to 4 slots — main + 3 thumbnails
  const [mainImg, ...extraImgs] = images
  const thumbnails = [0, 1, 2].map(i => extraImgs[i] ?? null)
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-14">
          ...
          <div className="space-y-3">
            ...
            <div className="relative w-full aspect-[4/5] rounded-2xl overflow-hidden bg-gray-100 shadow-sm">
              {mainImg ? (
                ...
                <GalleryPlaceholder accent={accent} label="Product image coming soon" />
              )}
            </div>
              ...
              {thumbnails.map((src, i) => (
                <div key={i} className="aspect-square rounded-xl overflow-hidden bg-gray-100">
                  {src ? (
                    ...
                    <GalleryPlaceholder accent={accent} label={i === 2 ? 'Lifestyle shot' : undefined} />
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="flex flex-col gap-6">
            ...
            {/* Size selector + dynamic price + CTA (client component) */}
            <SizeSelector
              sizes={product.sizes ?? []}
            ...
            {product.usage_instructions && (
              <div className="rounded-2xl p-4" style={{ backgroundColor: `${accent}08`, borderLeft: `3px solid ${accent}` }}>
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{product.usage_instructions}</p>
              </div>
            )}
