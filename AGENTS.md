...
| Task agent | Orchestrated by **Codex / Hermes / Codex** (best model per task) via the `oc-*` skills — **not** Groq |
...
Codex — to draft via the `oc-*` skills) vs `none` (manual). **The task agent
...
Any agent in this monorepo (Codex, Cowork, Codex, a worker) can drive the
...
- Full usage + workflows: **`.Codex/skills/oc-ops/SKILL.md`**.
...
- **`.Codex/skills/oc-design`** — brand guidelines, decks, posters, social kits,
  SVG/overlays. CLI: `node .Codex/skills/oc-design/scripts/oc-design.mjs`.
  Providers: Open Design (default) · Codex Design (handoff) · code-native.
- **`.Codex/skills/oc-video`** — code-native motion graphics, reels, talking-head,
  and general edit mode (HTML/CSS/SVG + Chrome + FFmpeg + Piper voice + WhisperX).
  CLI: `node .Codex/skills/oc-video/src/cli.mjs`. Six brand profiles under
...
best model in Codex / Hermes / Codex — never Groq.** Refresh from upstream
