...
export default function MediaUpload({ contentId }: { contentId: string }) {
  ...
  useEffect(() => {
    apiFetch<{ content: { assetUrls: string[] } }>(`/api/marketing/content?id=${contentId}`)
  ...
  async function onPick(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      for (const file of files) {
        ...
        fd.append('file', file)
        const res = await fetch(`/api/marketing/content/${contentId}/media`, {
          method: 'POST',
  ...
  async function remove(url: string) {
    try {
      const r = await apiFetch<{ assetUrls: string[] }>(`/api/marketing/content/${contentId}/media`, {
        method: 'DELETE',
