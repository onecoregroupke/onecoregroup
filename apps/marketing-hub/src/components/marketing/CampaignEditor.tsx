...
export default function CampaignEditor({ campaignId }: { campaignId?: string }) {
    ...
    void (async () => {
      ...
      try {
        ...
        if (campaignId) {
          ...
          }>(`/api/marketing/campaigns?id=${campaignId}`)
          hydrate(c)
          setContent(linked ?? [])
        } else if (b.brands?.[0]) {
  ...
  async function save() {
    ...
    try {
      ...
      if (campaignId) {
        const { campaign: c } = await apiFetch<{ campaign: MarketingCampaign }>('/api/marketing/campaigns', {
          method: 'PATCH',
            ...
            <div className="divide-y divide-gray-50">
              {content.length === 0 ? (
                ...
                content.map((c) => (
                  <Link
                    key={c.id}
                    href={`/marketing/content/${c.id}/edit`}
