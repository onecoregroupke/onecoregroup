...
/**
 * Branded transactional auth emails for the Marketing Hub, sent via Resend so
 * the links are fully controlled in code (we build a `/auth/callback?token_hash`
...
/** Resolve the public Hub URL (no trailing slash). */
export function hubUrl(): string {
  const raw = process.env['NEXT_PUBLIC_HUB_URL'] ?? 'http://localhost:3000'
  return raw.replace(/\/+$/, '')
}
  ...
  return `
    <div style="font-family: -apple-system, Segoe UI, sans-serif; max-width: 520px; margin: 0 auto; background: #f1f5f9; padding: 32px 16px;">
      <div style="text-align: center; margin-bottom: 24px;">
        ...
        <p style="margin: 12px 0 0; color: ${NAVY}; font-weight: 600;">One Core Group · Marketing Hub</p>
      </div>
...
function fromAddress(): string {
  // Reuse the verified Resend sender; label it for the Hub.
  const addr = process.env['EMAIL_FROM'] ?? 'noreply@nuuranest.com'
  return addr.includes('<') ? addr : `One Core Group Marketing Hub <${addr}>`
}
...
export async function sendInviteEmail(opts: { email: string; displayName?: string | null; link: string }) {
  ...
  await resend.emails.send({
    from: fromAddress(),
    to: opts.email,
    subject: 'You\'ve been invited to the One Core Group Marketing Hub',
    html: shell({
      heading: `Welcome${name} 👋`,
      body: 'An administrator has invited you to the One Core Group Marketing Hub. Click below to choose a password and activate your account — you\'ll use that password every time you sign in, on any device. This link expires in 24 hours.',
...
/** Send a branded password-reset email that lands the user on the set-password page. */
export async function sendRecoveryEmail(opts: { email: string; link: string }) {
  ...
  await resend.emails.send({
    from: fromAddress(),
    to: opts.email,
    subject: 'Reset your One Core Group Marketing Hub password',
    html: shell({
      heading: 'Reset your password',
      body: 'We received a request to reset the password for your Marketing Hub account. Click below to choose a new one. This link expires in 1 hour. If you didn\'t request this, you can safely ignore this email — your password won\'t change.',
