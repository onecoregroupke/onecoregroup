...
export default function NptAdminPage() {
  ...
  async function loadPianos() {
    ...
    try {
      const res = await fetch('/api/npt', { headers: await authHeaders() })
  ...
  async function exportCSV() {
    try {
      const headers = await authHeaders()
      const res = await fetch('/api/npt/export', { headers })
  ...
  async function importCSV(file: File) {
    ...
    try {
      ...
      const headers = await authHeaders()
      const res = await fetch('/api/npt/import', {
        method: 'POST',
              ...
              Object.entries(grouped).map(([cat, catPianos]) => (
                <div key={cat}>
                  ...
                  {catPianos.map(p => {
                    ...
                    const isRelative = mainImg?.startsWith('/')
                    return (
                      <button
                        key={p.id}
