  ...
  async function load() {
    ...
    try {
      ...
      const [b, c] = await Promise.all([
        ...
        apiFetch<{ content: MarketingContent[] }>(`/api/marketing/content?${params.toString()}`),
      ])
      setBrands(b.brands ?? [])
      setItems(c.content ?? [])
    } catch (err) {
              ...
              items.map((c) => (
                <tr key={c.id} className="hover:bg-gray-50">
                  <td className="px-5 py-3">
                    <Link href={`/marketing/content/${c.id}/edit`} className="font-medium text-gray-900 hover:text-ocg-navy">
