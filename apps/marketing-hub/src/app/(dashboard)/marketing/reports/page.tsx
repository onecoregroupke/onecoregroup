...
export default function ReportsPage() {
  ...
  async function load() {
    ...
    setError('')
    try {
      const { reports: r } = await apiFetch<{ reports: ExecutiveReport[] }>('/api/marketing/reports')
      setReports(r ?? [])
    } catch (err) {
  ...
  async function generate() {
    ...
    setError('')
    try {
      const { report } = await apiFetch<{ report: ExecutiveReport }>('/api/marketing/reports', {
        method: 'POST',
        body: JSON.stringify({ periodStart, periodEnd }),
      })
      router.push(`/marketing/reports/${report.id}`)
    } catch (err) {
              ...
              reports.map((r) => (
                <tr key={r.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-3">
                    <Link href={`/marketing/reports/${r.id}`} className="font-medium text-gray-900 hover:text-ocg-navy">
                      {r.subject}
                    </Link>
                  </td>
                  <td className="px-5 py-3">
