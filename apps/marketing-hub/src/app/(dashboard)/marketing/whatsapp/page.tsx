    ...
    void (async () => {
      ...
      try {
        const [f, b] = await Promise.all([
          apiFetch<{ flows: WhatsappFlow[] }>('/api/marketing/whatsapp?includeArchived=true'),
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        {canEdit && (
          <Link href="/marketing/whatsapp/new" className="inline-flex items-center gap-2 rounded-lg bg-ocg-navy px-4 py-2 text-sm font-medium text-white hover:bg-slate-800">
              ...
              flows.map((f) => (
                <tr key={f.id} className="hover:bg-gray-50">
                  <td className="px-5 py-3">
                    <Link href={`/marketing/whatsapp/${f.id}`} className="font-medium text-gray-900 hover:text-ocg-navy">{f.name}</Link>
