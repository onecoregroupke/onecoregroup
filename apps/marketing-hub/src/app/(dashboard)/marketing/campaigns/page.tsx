  ...
  async function load() {
    ...
    try {
      ...
      const [b, c] = await Promise.all([
        brands.length ? Promise.resolve({ brands }) : apiFetch<{ brands: MarketingBrand[] }>('/api/marketing/brands'),
        apiFetch<{ campaigns: MarketingCampaign[] }>(`/api/marketing/campaigns?${params.toString()}`),
      ])
      setBrands(b.brands ?? [])
              ...
              campaigns.map((c) => (
                <tr key={c.id} className="hover:bg-gray-50">
                  <td className="px-5 py-3">
                    <Link href={`/marketing/campaigns/${c.id}`} className="font-medium text-gray-900 hover:text-ocg-navy">
                      {c.name}
                    </Link>
                  </td>
