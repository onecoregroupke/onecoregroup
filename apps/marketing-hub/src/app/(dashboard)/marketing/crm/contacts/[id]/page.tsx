...
export default function ContactDetailPage() {
  ...
  async function saveContact() {
    ...
    try {
      const { contact: c } = await apiFetch<{ contact: MarketingContact }>('/api/marketing/contacts', {
        method: 'PATCH',
        body: JSON.stringify({
          id,
  ...
  async function logActivity() {
    ...
    setError('')
    try {
      await apiFetch('/api/marketing/activities', {
        method: 'POST',
  ...
  return (
    <div className="space-y-6">
      <Link href="/marketing/crm" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700">
