...
export default function PromoteLeadsPage() {
  ...
  async function load() {
    ...
    setError('')
    try {
      const { leads: l } = await apiFetch<{ leads: LeadToPromote[] }>('/api/marketing/leads')
      setLeads(l ?? [])
    } catch (err) {
  ...
  async function promote(leadId: string) {
    ...
    setError('')
    try {
      const { contact } = await apiFetch<{ contact: MarketingContact }>('/api/marketing/leads', {
        method: 'POST',
        body: JSON.stringify({ leadId }),
      })
      router.push(`/marketing/crm/contacts/${contact.id}`)
    } catch (err) {
