  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-lg border border-gray-100 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-end gap-3">
        ...
        <Field label="Status"><select className="input" value={status} onChange={(event) => setStatus(event.target.value)}><option value="">All</option><option value="open">In now</option><option value="capped">Missing checkout</option><option value="late">Late</option><option value="complete">Complete</option><option value="incomplete">Incomplete</option><option value="auto">Auto-closed</option></select></Field>
  ...
  return <section className="overflow-hidden rounded-lg border border-gray-100 bg-white shadow-sm">
    <div className="overflow-x-auto">
      <table className="w-full min-w-[980px] text-sm">
        ...
        <tbody className="divide-y divide-gray-50">{records.map((record) => <tr key={record.id} className={selectedId === record.id ? 'bg-amber-50' : 'hover:bg-gray-50'}><td className="px-4 py-3"><button onClick={() => onSelect(record)} className="text-left"><span className="font-medium text-gray-900">{record.employee_name}</span><span className="block text-xs text-gray-400">{record.employee_email || record.employee_code || record.attendance_date}</span></button></td><td className="px-4 py-3 text-gray-600">{time(record.scheduled_start_at)} - {time(record.scheduled_end_at)}</td><td className="px-4 py-3 text-gray-600"><span className="block">In {time(record.check_in_at)}</span><span className="block text-xs text-gray-400">Out {outLabel(record)}</span></td><td className="px-4 py-3 text-gray-600">{duration(record.effective_actual_minutes)}</td><td className="px-4 py-3"><Variance minutes={record.time_variance_minutes} /></td><td className="px-4 py-3"><StatusPill record={record} /></td></tr>)}</tbody>
      </table>
    </div>
  </section>
}
...
function RecordDetail({ record, evidence, onClose }: { record: AttendanceConsoleRecord; evidence: AttendanceEvidenceDaily[]; onClose: () => void }) {
  return <section className="rounded-lg border border-gray-100 bg-white p-5 shadow-sm">
    ...
    <div className="mt-4 grid gap-3 md:grid-cols-4"><Meta label="Expected IN" value={time(record.scheduled_start_at)} /><Meta label="Expected OUT" value={time(record.scheduled_end_at)} /><Meta label="Actual IN" value={`${time(record.check_in_at)} ${record.check_in_label ? `· ${record.check_in_label}` : ''}`} /><Meta label="Actual OUT" value={outLabel(record)} /><Meta label="Worked" value={duration(record.effective_actual_minutes)} /><Meta label="Expected duration" value={duration(record.expected_minutes)} /><Meta label="Late" value={duration(record.late_minutes)} /><Meta label="Early departure" value={duration(record.early_departure_minutes)} /><Meta label="Verified overtime" value={duration(record.overtime_minutes)} /><Meta label="Time variance" value={signedDuration(record.time_variance_minutes)} /><Meta label="Status" value={recordStatusText(record)} /><Meta label="Punch count" value={String(record.punch_count)} />{record.auto_close_processed_at ? <Meta label="Auto-close processed" value={time(record.auto_close_processed_at)} /> : null}</div>
...
function StatusPill({ record }: { record: AttendanceConsoleRecord }) { const text = recordStatusText(record); const cls = record.open_now ? 'bg-emerald-50 text-emerald-700' : record.is_hours_capped ? 'bg-orange-50 text-orange-700' : record.late_minutes > 0 ? 'bg-amber-50 text-amber-700' : record.status === 'incomplete' ? 'bg-red-50 text-red-700' : 'bg-gray-100 text-gray-700'; return <span className={`inline-flex items-center gap-1 rounded px-2 py-1 text-xs font-semibold ${cls}`}>{record.late_minutes > 0 || record.is_checkout_missing ? <TriangleAlert size={12} /> : <Clock3 size={12} />}{text}</span> }
...
function statusFor(record: AttendanceConsoleRecord) { if (record.open_now) return 'open'; if (record.is_hours_capped) return 'capped'; if (record.is_auto_closed) return 'auto'; if (record.status === 'incomplete') return 'incomplete'; if (record.late_minutes > 0) return 'late'; if (record.check_in_at && record.check_out_at) return 'complete'; return record.status }
...
function cellClass(record: AttendanceConsoleRecord) { if (record.open_now) return 'bg-emerald-100 text-emerald-800'; if (record.is_hours_capped) return 'bg-orange-100 text-orange-800'; if (record.is_auto_closed) return 'bg-blue-100 text-blue-800'; if (record.late_minutes > 0) return 'bg-amber-100 text-amber-800'; if (record.status === 'incomplete') return 'bg-red-100 text-red-800'; return 'bg-gray-800 text-white' }
function cellText(record: AttendanceConsoleRecord) { if (record.open_now) return 'I'; if (record.is_hours_capped) return 'C'; if (record.is_auto_closed) return 'A'; if (record.late_minutes > 0) return 'L'; if (record.status === 'incomplete') return '!'; return '✓' }
...
function recordStatusText(record: AttendanceConsoleRecord) { if (record.open_now) return 'IN NOW'; if (record.is_hours_capped) return `CHECKOUT MISSING · CAPPED AT ${time(record.effective_check_out_at)}`; if (record.is_auto_closed) return 'AUTO-CLOSED'; return record.status.toUpperCase() }
