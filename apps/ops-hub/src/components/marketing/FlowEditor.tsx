...
export default function FlowEditor({ flowId }: { flowId?: string }) {
  ...
  useEffect(() => {
    void (async () => {
      ...
      try {
        ...
        setBrands(b.brands ?? [])
        if (flowId) {
          const { flow: f } = await apiFetch<{ flow: WhatsappFlow }>(`/api/mhub/marketing/whatsapp?id=${flowId}`)
          hydrate(f)
        } else if (b.brands?.[0]) {
  ...
  async function save() {
    ...
    try {
      ...
      if (flowId) {
        const { flow: f } = await apiFetch<{ flow: WhatsappFlow }>('/api/mhub/marketing/whatsapp', {
          method: 'PATCH',
        ...
        router.replace(`/mhub/marketing/whatsapp/${f.id}`)
      }
    } catch (err) {
