...
export default function ContentEditor({ contentId, defaultDate, defaultPlatformId }: Props) {
    ...
    void (async () => {
      ...
      try {
        const [b, p, pl, cmp] = await Promise.all([
          ...
          apiFetch<{ campaigns: MarketingCampaign[] }>('/api/mhub/marketing/campaigns?status=any'),
        ])
        setBrands(b.brands ?? [])
        ...
        if (contentId) {
          const { content: c } = await apiFetch<{ content: MarketingContent }>(
            `/api/mhub/marketing/content?id=${contentId}`,
          )
          setContent(c)
  ...
  async function save() {
    ...
    try {
      ...
      if (contentId) {
        const { content: c } = await apiFetch<{ content: MarketingContent }>('/api/mhub/marketing/content', {
          method: 'PATCH',
        ...
        router.replace(`/mhub/marketing/content/${c.id}/edit`)
      }
    } catch (err) {
