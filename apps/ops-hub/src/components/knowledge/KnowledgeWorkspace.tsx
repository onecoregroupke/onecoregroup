...
import { BookOpenCheck, History, Plus, ShieldCheck } from 'lucide-react'
    ...
    <div className="space-y-6">
      ...
      {records.length === 0 ? <div className="rounded-xl border border-dashed border-gray-200 bg-white p-10 text-center"><BookOpenCheck className="mx-auto text-gray-300" size={36} /><p className="mt-3 font-medium text-gray-700">No knowledge entries in your scope</p><p className="mt-1 text-sm text-gray-500">Legacy documents will remain legacy until deliberately reviewed and replaced by an approved current version.</p></div> : (
        <div className="grid gap-4 xl:grid-cols-2">{records.map((record) => {
          ...
          return <article key={record.id} className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-gray-400">{record.knowledge_type.replaceAll('_', ' ')}</p><h2 className="mt-1 font-semibold text-gray-900">{record.title}</h2></div><Status value={shown?.status ?? 'draft'} /></div>
...
function Status({ value }: { value: string }) { const tone = value === 'current' ? 'bg-green-50 text-green-700' : value === 'legacy' ? 'bg-amber-50 text-amber-700' : value === 'draft' ? 'bg-blue-50 text-blue-700' : 'bg-gray-100 text-gray-600'; return <span className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase ${tone}`}>{value}</span> }
...
function VersionDialog({ record, busy, onClose, onSubmit }: { record: KnowledgeRecord; busy: boolean; onClose: () => void; onSubmit: (values: Record<string, unknown>) => Promise<boolean> }) {
