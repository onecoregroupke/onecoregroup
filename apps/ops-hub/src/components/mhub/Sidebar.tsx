...
const MAIN_NAV: { href: string; label: string; icon: React.ElementType; section: SectionKey }[] = [
  { href: '/mhub',            label: 'Dashboard',     icon: LayoutDashboard, section: 'dashboard'  },
  { href: '/mhub/input',       label: 'Input Portal',  icon: PenSquare,       section: 'input'      },
  { href: '/mhub/compliance',  label: 'Compliance',    icon: CheckSquare,     section: 'compliance' },
  { href: '/mhub/properties',  label: 'Properties',    icon: Home,            section: 'properties' },
  { href: '/mhub/glitz',       label: "Glitz N' Glim", icon: Sparkles,        section: 'glitz'      },
  { href: '/mhub/npt',         label: 'NPT Catalogue', icon: BarChart2,       section: 'npt'        },
  { href: '/mhub/reports',     label: 'Reports',       icon: FileText,        section: 'reports'    },
  { href: '/mhub/users',       label: 'Users',         icon: Users,           section: 'users'      },
]
...
const MARKETING_NAV: { href: string; label: string; icon: React.ElementType }[] = [
  ...
  { href: '/mhub/marketing/preview',   label: 'Preview',   icon: Eye       },
  { href: '/mhub/marketing/content',   label: 'Content',   icon: PenSquare },
  { href: '/mhub/marketing/episodes',  label: 'Episodes',  icon: Clapperboard },
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
          ...
          {showBrands && (
            <div className="pt-4 mt-2 border-t border-white/10">
              ...
              {BRANDS.map((brand) => {
                const href = `/mhub/brands/${brand.slug}`
                const active = path === href
                return (
                  <Link
                    key={brand.slug}
