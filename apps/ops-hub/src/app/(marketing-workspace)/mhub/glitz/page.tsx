...
export default function GlitzAdminPage() {
  ...
  async function loadProducts() {
    ...
    try {
      const res = await fetch('/api/mhub/glitz', { headers: await authHeaders() })
  ...
  async function saveProduct() {
    ...
    try {
      ...
      const res = await fetch('/api/mhub/glitz', {
  ...
  async function exportCSV() {
    try {
      const headers = await authHeaders()
      const res = await fetch('/api/mhub/glitz/export', { headers })
  ...
  async function importCSV(file: File) {
    ...
    try {
      ...
      const headers = await authHeaders()
      const res = await fetch('/api/mhub/glitz/import', {
        method: 'POST',
                        ...
                        <div className="flex gap-2.5 items-center">
                          <div className={`h-10 w-10 rounded-lg overflow-hidden flex items-center justify-center flex-shrink-0 ${active ? 'bg-white/10' : 'bg-gray-100'}`}>
                            {mainImg ? (
                              ...
                              <img src={mainImg.startsWith('/mhub') ? `${GLITZ_URL}${mainImg}` : mainImg}
                ...
                <div className="grid grid-cols-4 gap-2">
                  {linesToArray(form.images).map((url, i, arr) => (
                    ...
                    >
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img
                        src={url.startsWith('/mhub') ? `${GLITZ_URL}${url}` : url}
                        alt=""
