...
export default function ReportDetailPage() {
  ...
  async function load() {
    ...
    setLoading(true)
    try {
      const { report: r } = await apiFetch<{ report: ExecutiveReport }>(`/api/mhub/marketing/reports?id=${id}`)
      hydrate(r)
    } catch (err) {
  ...
  async function save() {
    ...
    try {
      const { report: r } = await apiFetch<{ report: ExecutiveReport }>('/api/mhub/marketing/reports', {
        method: 'PATCH',
        body: JSON.stringify({
          id,
  ...
  return (
    <div className="space-y-6">
      <Link href="/mhub/marketing/reports" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700">
