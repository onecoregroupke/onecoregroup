  ...
  async function load() {
    ...
    try {
      const [d, c] = await Promise.all([
        apiFetch<{ deals: MarketingDeal[] }>(`/api/mhub/marketing/deals?stage=${stageFilter || 'any'}`),
        apiFetch<{ contacts: MarketingContact[] }>('/api/mhub/marketing/contacts?stage=any'),
      ])
      setDeals(d.deals ?? [])
              ...
              deals.map((d) => (
                <tr key={d.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-3">
                    <Link href={`/mhub/marketing/crm/contacts/${d.contactId}`} className="text-gray-600 hover:text-ocg-navy">
