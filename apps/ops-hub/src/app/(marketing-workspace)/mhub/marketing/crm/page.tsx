...
export default function CrmContactsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const { contacts: c } = await apiFetch<{ contacts: MarketingContact[] }>(
        `/api/mhub/marketing/contacts?${params.toString()}`,
      )
      setContacts(c ?? [])
    } catch (err) {
  ...
  async function createContact() {
    ...
    setError('')
    try {
      await apiFetch('/api/mhub/marketing/contacts', {
        method: 'POST',
        body: JSON.stringify({
              ...
              contacts.map((c) => (
                <tr key={c.id} className="hover:bg-gray-50">
                  <td className="px-5 py-3">
                    <Link href={`/mhub/marketing/crm/contacts/${c.id}`} className="font-medium text-gray-900 hover:text-ocg-navy">
