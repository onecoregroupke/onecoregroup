...
export default async function DarulAdminTasksPage() {
  const { adminTasks, students } = await getDarulAdminData()
  const studentById = new Map(students.map((s) => [s.id, s.full_name]))
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-semibold text-gray-900">Darul Swafa admin tasks</h1>
      ...
      <section className="overflow-x-auto rounded-xl border border-gray-100 bg-white shadow-sm">
        {adminTasks.length === 0 ? (
          <p className="p-6 text-sm text-gray-500">No admin tasks yet.</p>
        ) : (
