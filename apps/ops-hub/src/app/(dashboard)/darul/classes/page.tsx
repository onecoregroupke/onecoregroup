...
export default async function DarulClassesPage() {
  const { classes, students, team } = await getDarulAdminData()
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-semibold text-gray-900">Darul Swafa halaqas</h1>
        <p className="text-sm text-gray-500">Halaqas (classes), their level, and the assigned teacher.</p>
      </div>
      <DarulQuickAdd />
      <section className="overflow-x-auto rounded-xl border border-gray-100 bg-white shadow-sm">
        {classes.length === 0 ? (
          <p className="p-6 text-sm text-gray-500">No halaqas yet.</p>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 text-left text-[11px] uppercase tracking-wider text-gray-400">
                <th className="px-4 py-3">Halaqa</th><th className="px-4 py-3">Level</th><th className="px-4 py-3">Teacher</th>
