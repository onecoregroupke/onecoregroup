...
export default async function DarulAdmissionsPage() {
  const { admissions, students, guardians } = await getDarulAdminData()
  const studentById = new Map(students.map((s) => [s.id, s.full_name]))
  const guardianById = new Map(guardians.map((g) => [g.id, g.full_name]))
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-semibold text-gray-900">Darul Swafa admissions</h1>
        <p className="text-sm text-gray-500">Enquiry-to-enrollment pipeline with follow-up dates.</p>
      </div>
      <DarulQuickAdd />
      <section className="overflow-x-auto rounded-xl border border-gray-100 bg-white shadow-sm">
        {admissions.length === 0 ? (
          <p className="p-6 text-sm text-gray-500">No admissions in the pipeline yet.</p>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 text-left text-[11px] uppercase tracking-wider text-gray-400">
                ...
                <th className="px-4 py-3">Documents</th><th className="px-4 py-3">Next follow-up</th><th className="px-4 py-3">Source</th>
              </tr>
            </thead>
