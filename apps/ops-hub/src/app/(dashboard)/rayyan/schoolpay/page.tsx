...
export default async function RayyanSchoolpayPage() {
  const { batches, snapshots, invoices, payments, students } = await getRayyanAdminData()
  ...
  return (
    <div className="space-y-5">
      <div><h1 className="text-2xl font-semibold text-gray-900">SchoolPay reconciliation</h1><p className="text-sm text-gray-500">Import SchoolPay snapshots and compare them with the internal manual fee ledger.</p></div>
      ...
      <section className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 overflow-hidden rounded-lg border border-gray-100">
          {invoices.length === 0 ? <p className="p-4 text-sm text-gray-500">No manual Rayyan invoices yet. Use Rayyan Admin actions to add one.</p> : (
