...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const supabase = db()
    const { data: studentsData } = await supabase.from('rayyan_students').select('*')
    const students = (studentsData as RayyanStudentRow[] | null) ?? []
    ...
    const { error: insertError } = await supabase.from('rayyan_schoolpay_payment_snapshots').insert(snapshots)
    if (insertError) throw new Error(insertError.message)

    const followups = snapshots
      .filter((s) => Number(s.balance_ksh ?? 0) > 0)
      .map((s) => ({
        student_id: s.student_id,
        schoolpay_code: s.schoolpay_code,
        expected_fee_item: s.fee_item || 'SchoolPay balance',
        follow_up_status: 'pending',
        last_known_fee_status: s.payment_status,
        next_follow_up_date: null,
        notes: `Imported balance: KSh ${Number(s.balance_ksh ?? 0).toLocaleString()}`,
      }))
    if (followups.length > 0) {
      await supabase.from('rayyan_fee_followups').insert(followups)
    }
