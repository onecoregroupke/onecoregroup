...
If Google Drive Desktop is not connected, guide the setup first. If work must continue, use the relevant skill's local fallback folder (`.Codex/skills/<skill>/projects/TASK-XXXX/03_Working-Files`) and attach that fallback path to Task Ops with `attach-context`.
