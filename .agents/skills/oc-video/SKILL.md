---
name: oc-video
description: Code-native One Core Group video production for the Ops Hub. Use when Codex, Hermes, or Codex needs to create motion-graphics videos, reels, video overlays, transparent graphics, talking-head enhancements, captions, audio mixes, FFmpeg renders, or draft video deliverables for an OCG TASK-XXXX (across the 6 brands). Uses HTML/CSS/SVG/JS, local Chrome/Chromium, FFmpeg, manually supplied audio, and oc-ops/Ops Hub delivery. Never use Canva, AI image generation, AI video generation, automatic publishing, or external sending.
---
...
**Model note:** this skill is executed by the orchestrating agent (Codex / Hermes / Codex) using the best available model — NOT by Groq. Groq is reserved for the Ops Hub's automated daily/weekly/monthly report narration only.
...
```bash
node .Codex/skills/oc-video/src/cli.mjs <command> [options]
```
...
The repo copy of this skill is canonical. On a new device, use it directly from `.Codex/skills/oc-video`; configure `OPS_OPS_BASE_URL`, `OPS_AGENT_API_KEY`, and `OCG_LOCAL_DELIVERY_ROOT` in `.env.local` or the environment.
...
```bash
node .Codex/skills/oc-video/src/cli.mjs init --task TASK-XXXX --mode motion_graphics --brand glitz-n-glim
node .Codex/skills/oc-video/src/cli.mjs plan --task TASK-XXXX
node .Codex/skills/oc-video/src/cli.mjs voice-script --task TASK-XXXX
node .Codex/skills/oc-video/src/cli.mjs generate-voice --task TASK-XXXX --voice wm_voice_female
node .Codex/skills/oc-video/src/cli.mjs align-voice --task TASK-XXXX
node .Codex/skills/oc-video/src/cli.mjs generate-scenes --task TASK-XXXX
node .Codex/skills/oc-video/src/cli.mjs render-scenes --task TASK-XXXX
node .Codex/skills/oc-video/src/cli.mjs build-manifest --task TASK-XXXX
node .Codex/skills/oc-video/src/cli.mjs compile --task TASK-XXXX
node .Codex/skills/oc-video/src/cli.mjs deliver --task TASK-XXXX
```
...
If Drive sync is unavailable, deliver into `.Codex/skills/oc-video/projects/TASK-XXXX/03_Working-Files`, label it as a local fallback, and attach that fallback path to Task Ops when credentials are available.
