---
name: oc-design
description: One Core Group design-production routing for the Ops Hub. Use when Codex, Hermes, or Codex needs to create brand guidelines, brand/design systems, pitch/brand decks, posters, social-media design kits, SVG/transparent graphics, overlays, or local design deliverables for an OCG TASK-XXXX (across the 6 brands). Defaults to Open Design as provider and delivers draft files through oc-ops / Ops Hub. Draft-only: never publish or send externally.
---
...
**Model note:** this skill is executed by the orchestrating agent (Codex / Hermes / Codex) using the best available model — NOT Groq. Groq is reserved for the Ops Hub's automated report narration.
...
```bash
node .Codex/skills/oc-design/scripts/oc-design.mjs <command> [options]
```

Default provider is `open_design`. Use **oc-ops** for task lookup/status/context and Ops Hub delivery for files.

The repo copy of this skill is canonical. On a new device, use it directly from `.Codex/skills/oc-design`; configure `OPS_OPS_BASE_URL`, `OPS_AGENT_API_KEY`, and `OCG_LOCAL_DELIVERY_ROOT` in `.env.local` or the environment.

## Core Workflow

```bash
node .Codex/skills/oc-design/scripts/oc-design.mjs doctor
node .Codex/skills/oc-design/scripts/oc-design.mjs init --task TASK-XXXX --type poster --brand glitz-n-glim
node .Codex/skills/oc-design/scripts/oc-design.mjs route --task TASK-XXXX
node .Codex/skills/oc-design/scripts/oc-design.mjs collect --task TASK-XXXX --source "<export-folder-or-file>"
node .Codex/skills/oc-design/scripts/oc-design.mjs deliver --task TASK-XXXX
```
...
- `claude_design`: Handoff-only until Codex Design exposes a product API. Prepare prompts/checklists, then collect manually downloaded files.
...
- If Drive sync is unavailable, deliver into `.Codex/skills/oc-design/projects/TASK-XXXX/03_Working-Files`, label it as a local fallback, and attach that fallback path to Task Ops when credentials are available.
